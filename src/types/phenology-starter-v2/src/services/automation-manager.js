...
export function buildLightOnAutomation(stage) {
  ...
  return {
    ...
    description: `Turn on grow light at ${stage.lightSchedule.onTime} for ${stage.name} stage. Auto-generated by GrowOp Dashboard.`,
    trigger: [
      {
        platform: 'time',
...
export function buildLightOffAutomation(stage) {
  ...
  return {
    id: AUTOMATION_IDS.LIGHT_OFF,
    alias: `Phenology: Lights OFF (${stage.name})`,
    description: `Turn off grow light at ${stage.lightSchedule.offTime} for ${stage.name} stage. Auto-generated by GrowOp Dashboard.`,
    trigger: [
      {
        platform: 'time',
...
export async function deployStageAutomations(stage, options = {}) {
  ...
  for (const config of configs) {
    try {
      console.log(`[AUTO-MGR] Deploying: ${config.alias}`);
      await createAutomation(config.id, config);
      deployed.push({ id: config.id, alias: config.alias });
    } catch (err) {
      console.error(`[AUTO-MGR] Failed to deploy ${config.id}:`, err.message);
      failed.push({ id: config.id, alias: config.alias, error: err.message });
    }
  }
  ...
  try {
    ...
    console.error('[AUTO-MGR] Failed to reload automations:', err.message);
  }
